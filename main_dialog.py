...
from bots.report_generator import ReportBot
...
class MainDialog(ComponentDialog):
    ...
    async def act_step(self, step_context: WaterfallStepContext) -> DialogTurnResult:
        ...
        if intent == Intent.GENERATE_REPORT.value:
            #get_weather_text = "TODO: get weather flow here"
            #get_weather_message = MessageFactory.text(
             #   get_weather_text, get_weather_text, InputHints.ignoring_input
            #)
    ...
    async def final_step(self,step_context: WaterfallStepContext) -> DialogTurnResult:
        ...
        if step_context.result is not None:
            ...
            if hasattr(result, 'typeofreport'):

            # Now we have all the booking details call the booking service.
                 
            # how to get the user_query from the search dialog
                user_query = "Generate report for : " + result.typeofreport + " " + result.timeperiod + " " + result.reportformat
                
                search_results = search_data(user_query)
                
                #search_summary = " ".join([doc['content'] for doc in search_results])
                gpt_prompt = f"Based on the following information, respond to the query: {user_query} for saving in pdf format\n\n{search_results}"
            ...
            # Step 3: Get GPT-generated response
                ...
                # time_property = Timex(result.travel_date)
                # travel_date_msg = time_property.to_natural_language(datetime.now())
                new_prompt = f"Create a dictionary (key-value) pair for parsing into a data frame for the following: {gpt_response}"
                new_prompt_response = generate_gpt_response(new_prompt)
                print("-------------------------")
                print(new_prompt_response)
                print("-------------------------")
                msg_txt = response_message
                print(msg_txt)
                await ReportBot.CreateReport(step_context.context,new_prompt_response)
                message = MessageFactory.text(msg_txt, msg_txt, InputHints.ignoring_input)
                await step_context.context.send_activity(message)    
    ...
    async def _show_warning_for_unsupported_cities(
        context: TurnContext, luis_result: SearchDetails
    ) -> None:
